...
import com.reference.implementation.data.mappers.toMessageRequestDto
...
@Singleton
class MessageCacheRepositoryImpl @Inject constructor(
    ...
    override suspend fun markMessageAsRead(messageId: Int, onRetry: suspend (Int) -> Unit) {
        val response = withContext(ioDispatcher) {
            try {
                ...
                withContext(NonCancellable) {
                    auditLog("${auditLogTimestamp()} mark message as read ended")
                }
            }
        }
        ...
        if (originalState is NetworkResult.Success) { // due diligence - proceed when previously got data
            ...
            withContext(ioDispatcher) {
                try {
                    ...
                    withContext(NonCancellable) {
                        auditLog("${auditLogTimestamp()} delete message ended")
                    }
                }
            }
        }
    ...
    override suspend fun restoreMessage(
        ...
        withContext(ioDispatcher) {
            try {
                ...
                withContext(NonCancellable) {
                    auditLog("${auditLogTimestamp()} restore message ended")
                }
            }
        }
    }
